...
public class explosion_fx : MonoBehaviour {
    ...
    int myGarbageId;
    GarbageManager myGarbageManager;
    GarbageManager.fxGarbageType myGrabageType;
	...
	public void Activate_me(GarbageManager garbage, GarbageManager.fxGarbageType grabageType, int id = -99) //call this the the first time, when tile create the fx
		{
        myGarbageManager = garbage;
        myGrabageType = grabageType;
        myGarbageId = id;
	...
	void Go_to_garbage()
		{
        if (myGrabageType == GarbageManager.fxGarbageType.GemExplosion)
            myGarbageManager.garbage_recycle_gem_explosion_fx[myGarbageId].Add(this.gameObject);
        else if (myGrabageType == GarbageManager.fxGarbageType.BigExplosion)
            myGarbageManager.garbage_recycle_big_explosion_fx[myGarbageId].Add(this.gameObject);
        else if (myGrabageType == GarbageManager.fxGarbageType.BonusDestroyOne)
            myGarbageManager.garbage_recycle_bonus_destroy_one_fx.Add(this.gameObject);
        else if (myGrabageType == GarbageManager.fxGarbageType.BonusDestroyHorizontal)
            myGarbageManager.garbage_recycle_bonus_destroy_horizontal_fx.Add(this.gameObject);
        else if (myGrabageType == GarbageManager.fxGarbageType.BonusDestroyVertical)
            myGarbageManager.garbage_recycle_bonus_destroy_vertical_fx.Add(this.gameObject);
        else if (myGrabageType == GarbageManager.fxGarbageType.BonusDestroyHorizontalAndVertical)
            myGarbageManager.garbage_recycle_bonus_destroy_horizontal_and_vertical_fx.Add(this.gameObject);

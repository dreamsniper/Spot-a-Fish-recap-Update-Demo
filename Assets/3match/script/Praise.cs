...
using System.Collections;
	...
	public void Big_explosion(int explosion_magnitude)
	{
		...
		if (big_explosion_praise_sfx.Length > 0)
		{
			...
			if ((explosion_magnitude-adjust_count) > big_explosion_praise_sfx.Length)
				board.audioManager.Play_sfx(big_explosion_praise_sfx[big_explosion_praise.Length-1]);
			else
				board.audioManager.Play_sfx(big_explosion_praise_sfx[explosion_magnitude-adjust_count-1]);
		}
	...
	public void Combo_secondary_explosion(int combo_length)
	{
		...
		if (combo_secondary_explosion_praise_sfx.Length > 0)
		{
			if ((combo_secondary_explosion_praise.Length) > combo_secondary_explosion_praise_sfx.Length)
				board.audioManager.Play_sfx(combo_secondary_explosion_praise_sfx[combo_secondary_explosion_praise_sfx.Length-1]);
			else
				board.audioManager.Play_sfx(combo_secondary_explosion_praise_sfx[combo_length-1]);
		}
	...
	public void Combo_color(int combo_length)
	{
		...
		if (combo_color_praise_sfx.Length > 0)
		{
			if ((combo_color_praise.Length) > combo_color_praise_sfx.Length)
				board.audioManager.Play_sfx(combo_color_praise_sfx[combo_color_praise.Length-1]);
			else
				board.audioManager.Play_sfx(combo_color_praise_sfx[combo_length-1]);
		}
	...
	public void Win_and_continue_to_play(int n_star)
	{
		...
		if (board.globalRules.for_gain_a_star)
			{
			my_text.text = get_a_star[n_star];
			if (get_a_star_sfx.Length > 0)
				{
				if ((n_star) > get_a_star_sfx.Length)
					board.audioManager.Play_sfx(get_a_star_sfx[get_a_star_sfx.Length-1]);
				else
					board.audioManager.Play_sfx(get_a_star_sfx[n_star]);
				}
	...
	public void Gain_a_turn(string name, int quantity)
	{
		...
		if (gain_a_turn_sfx)
			board.audioManager.Play_sfx(gain_a_turn_sfx);
